...
{
    /// <summary>
    /// Defines a most recently used (MRU) cache replacement policy.
    /// </summary>
    ...
    public sealed class MostRecentlyUsedReplacementPolicy<TKey, TValue> : IReplacementPolicy<TKey, TValue>
    {
        /// <summary>
        ...
        /// <remarks>
        /// <para>This method applies the policy to return the item with the latest
        /// <see cref="CacheItem{TKey,TValue}.LastAccessedDate"/>. If two items were
        /// last accessed at the same time, and that is the latest time of the items
        ...
        public TKey GetItemToReplace(ICollection<CacheItem<TKey, TValue>> cacheSet)
        {
            ...
            if (cacheSet != null)
            {
                foreach (CacheItem<TKey, TValue> item in cacheSet)
                {
                    ...
                    {
                        // The current item was used more recently than the candidate,
